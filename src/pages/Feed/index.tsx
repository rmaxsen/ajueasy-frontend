...
import { Heart, MessageCircle, Plus, Tag, Loader2, Send } from 'lucide-react'
...
export default function FeedPage() {
  ...
  useEffect(() => {
    feedApi.list()
      .then((res) => setPosts(Array.isArray(res) ? res : (res as any).data ?? []))
      .catch(() => setPosts([]))
            ...
            <DialogContent className="max-w-lg">
              ...
              <form onSubmit={handleSubmit(onCreatePost)} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea placeholder="Escreva seu artigo..." rows={6} {...register('content', { required: true })} />
                </div>
                <div className="space-y-1.5">
                  <Label>Tags (separadas por vírgula)</Label>
                  <div className="relative">
                    <Tag className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input className="pl-9" placeholder="Direito Civil, Contratos, Dicas" {...register('tags')} />
                  </div>
                </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  <Avatar className="h-10 w-10">
                    ...
                    <AvatarFallback>{post.author ? getInitials(post.author.name) : '?'}</AvatarFallback>
                  </Avatar>
                  <div>
                    <Link
                ...
                <Link to={`/feed/${post.id}`}>
                  <h2 className="text-lg font-semibold mb-2 hover:text-primary transition-colors">{post.title}</h2>
                </Link>
                ...
                <div className="flex items-center gap-4 pt-3 border-t">
                  ...
                  <Link to={`/feed/${post.id}`} className="ml-auto text-xs text-primary hover:underline">
                    Ler artigo completo
                  </Link>
                </div>
